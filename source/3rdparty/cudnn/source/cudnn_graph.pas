...
const
  ...
  libname = 'cudnn64_9.dll';

  CUDNN_MAJOR = 9 ;
  CUDNN_MINOR = 5 ;
  CUDNN_PATCHLEVEL = 0 ;

  CUDNN_VERSION = (CUDNN_MAJOR * 10000 + CUDNN_MINOR * 100 + CUDNN_PATCHLEVEL) ;



  CUDNN_MAX_SM_MAJOR_NUMBER = 9  ;
  CUDNN_MAX_SM_MINOR_NUMBER = 0  ;
  CUDNN_MAX_DEVICE_VERSION = (CUDNN_MAX_SM_MAJOR_NUMBER * 100 + CUDNN_MAX_SM_MINOR_NUMBER * 10) ;


type
{$if not declared(cudaStream_t)}
  PcudaStream_t = ^cudaStream_t;
  cudaStream_t = IntPtr;
{$endif}
  PcudnnHandle_t = ^cudnnHandle_t;
  cudnnHandle_t = ^cudnnContext;
  cudnnContext = record end;

  PcudnnStatus_t = ^cudnnStatus_t;
  cudnnStatus_t=  (
...
const
    ...
    CUDNN_STATUS_RUNTIME_PREREQUISITE_MISSING = CUDNN_STATUS_NOT_SUPPORTED_RUNTIME_PREREQUISITE_MISSING ;
    CUDNN_STATUS_VERSION_MISMATCH = CUDNN_STATUS_SUBLIBRARY_VERSION_MISMATCH ;
type
(* human-readable error messages *)

PcudnnErrQueryMode_t = ^cudnnErrQueryMode_t;
cudnnErrQueryMode_t = (
 ...
 PcudnnDataType_t = ^cudnnDataType_t;
 cudnnDataType_t = (
    CUDNN_DATA_FLOAT                         = 0,
    CUDNN_DATA_DOUBLE                        = 1,
    CUDNN_DATA_HALF                          = 2,
 ...
 * CUDNN math type
 *)
 PcudnnMathType_t = ^cudnnMathType_t;
 cudnnMathType_t = (
 ...
 *)
 ...
 PcudnnNanPropagation_t = ^cudnnNanPropagation_t;
 ...
 *)
 ...
 PcudnnCTCGradMode_t = ^cudnnCTCGradMode_t;
 cudnnCTCGradMode_t =(
    CUDNN_CTC_ZERO_OOB_GRADIENTS = 0,
    CUDNN_CTC_SKIP_OOB_GRADIENTS = 1
) ;

 PcudnnTensorFormat_t = ^cudnnTensorFormat_t ;
 cudnnTensorFormat_t = (
 ...
 *)
 ...
 PcudnnReduceTensorOp_t = ^cudnnReduceTensorOp_t;
 ...
 *)
 ...
 PcudnnActivationMode_t = ^cudnnActivationMode_t;
 ...
 PcudnnSeverity_t = ^cudnnSeverity_t;
 cudnnSeverity_t = (
    CUDNN_SEV_FATAL   = 0,
    CUDNN_SEV_ERROR   = 1,
    CUDNN_SEV_WARNING = 2,
 ...
 *)
 ...
 PcudnnConvolutionMode_t = ^cudnnConvolutionMode_t;
 cudnnConvolutionMode_t = ( CUDNN_CONVOLUTION = 0, CUDNN_CROSS_CORRELATION = 1 ) ;

(*
 * CUDNN Reorder
 *)

 PcudnnReorderType_t = ^cudnnReorderType_t;
 cudnnReorderType_t = (
    CUDNN_DEFAULT_REORDER = 0,
    CUDNN_NO_REORDER      = 1
)  ;

 PcudnnPointwiseMode_t = ^cudnnPointwiseMode_t;
 ...
 PcudnnResampleMode_t =  ^cudnnResampleMode_t;
 cudnnResampleMode_t = (
 ...
 PcudnnSignalMode_t = ^cudnnSignalMode_t;
 cudnnSignalMode_t = (
    CUDNN_SIGNAL_SET  = 0,
    CUDNN_SIGNAL_WAIT = 1
) ;

 PcudnnGenStatsMode_t = ^cudnnGenStatsMode_t;
 cudnnGenStatsMode_t = (
    CUDNN_GENSTATS_SUM_SQSUM = 0
) ;

 PcudnnBnFinalizeStatsMode_t = ^cudnnBnFinalizeStatsMode_t;
 cudnnBnFinalizeStatsMode_t = (
    CUDNN_BN_FINALIZE_STATISTICS_TRAINING  = 0,
    CUDNN_BN_FINALIZE_STATISTICS_INFERENCE = 1
) ;

 PcudnnRngDistribution_t = ^cudnnRngDistribution_t;
 ...
 PcudnnBackendAttributeName_t = ^cudnnBackendAttributeName_t;
 ...
 PcudnnBackendAttributeType_t = ^cudnnBackendAttributeType_t;
 ...
 PcudnnBackendDescriptorType_t =  ^cudnnBackendDescriptorType_t;
 ...
 PcudnnBackendNumericalNote_t =  ^cudnnBackendNumericalNote_t;
 ...
 PcudnnBackendBehaviorNote_t = ^cudnnBackendBehaviorNote_t;
 ...
 PcudnnBackendKnobType_t = ^cudnnBackendKnobType_t;
 ...
 PcudnnBackendLayoutType_t = ^cudnnBackendLayoutType_t;
 ...
 PcudnnBackendHeurMode_t = ^cudnnBackendHeurMode_t;
 ...
 PcudnnBackendTensorReordering_t = ^cudnnBackendTensorReordering_t;
 ...
 PcudnnPaddingMode_t = ^cudnnPaddingMode_t;
 cudnnPaddingMode_t = (
    CUDNN_ZERO_PAD     = 0,
    CUDNN_NEG_INF_PAD  = 1,
 ...
 PcudnnBackendNormMode_t = ^cudnnBackendNormMode_t;
 ...
 PcudnnBackendNormFwdPhase_t = ^cudnnBackendNormFwdPhase_t;
 cudnnBackendNormFwdPhase_t = (
    CUDNN_NORM_FWD_INFERENCE = 0,
    CUDNN_NORM_FWD_TRAINING  = 1
) ;

{$if not declared(libraryPropertyType)}
 libraryPropertyType =
 (
 	MAJOR_VERSION,
 	MINOR_VERSION,
 ...
 {$endif}
 {$if not declared(cudaGraph_t)}
   cudaGraph_t = pointer;
 {$endif}
 PcudnnBackendDescriptor_t = ^cudnnBackendDescriptor_t;
 cudnnBackendDescriptor_t = pointer;

 pcudnnRuntimeTag_t = ^cudnnRuntimeTag_t;
 cudnnRuntimeTag_t = record end;

 PcudnnDebug_t = ^cudnnDebug_t;
 cudnnDebug_t = record
      cudnn_version : longword;
      ...
      pid : uint64;
      tid : uint64;
      cudaDeviceId : longint;
      reserved : array [0..14] of longint;
 end ;

 PcudnnFraction_t = ^cudnnFraction_t;
 cudnnFraction_t = record
    numerator : int64;
    denominator: int64;
 end;

  PcudnnCallback_t = ^cudnnCallback_t;
  cudnnCallback_t = procedure( sev : cudnnSeverity_t; udata : pointer ; const dbg : PcudnnDebug_t; const msg : PAnsiChar);


const
  CUDNN_SEV_ERROR_EN  = (1 shl ord(CUDNN_SEV_ERROR));
  CUDNN_SEV_WARNING_EN  = (1 shl ord(CUDNN_SEV_WARNING));
  CUDNN_SEV_INFO_EN  = (1 shl ord(CUDNN_SEV_INFO));

  function cudnnGetVersion():SizeInt; winapi; external libname;
  function cudnnGetMaxDeviceVersion():SizeInt; winapi; external libname;
  function cudnnGetCudartVersion():SizeInt; winapi; external libname;
  function cudnnGetErrorString(status: cudnnStatus_t):PAnsiChar; winapi; external libname;
  procedure cudnnGetLastErrorString(message: PAnsiChar; max_size: SizeInt); winapi; external libname;
  function cudnnQueryRuntimeError(handle: cudnnHandle_t; rstatus: PcudnnStatus_t; mode: cudnnErrQueryMode_t; tag: PcudnnRuntimeTag_t):cudnnStatus_t; winapi; external libname;
  function cudnnGetProperty(&type: libraryPropertyType; value: Plongint):cudnnStatus_t; winapi; external libname;
  function cudnnCreate(handle: PcudnnHandle_t):cudnnStatus_t; winapi; external libname;
  function cudnnDestroy(handle: cudnnHandle_t):cudnnStatus_t; winapi; external libname;
  function cudnnSetStream(handle: cudnnHandle_t; streamId: cudaStream_t):cudnnStatus_t; winapi; external libname;
  function cudnnGetStream(handle: cudnnHandle_t; streamId: PcudaStream_t):cudnnStatus_t; winapi; external libname;
  function cudnnSetCallback(mask: longword; udata: Pointer; fptr: cudnnCallback_t):cudnnStatus_t; winapi; external libname;
  function cudnnGetCallback(mask: Plongword; udata: PPointer; fptr: PcudnnCallback_t):cudnnStatus_t; winapi; external libname;
  function cudnnGraphVersionCheck():cudnnStatus_t; winapi; external libname;
  function cudnnBackendCreateDescriptor(descriptorType: cudnnBackendDescriptorType_t; descriptor: PcudnnBackendDescriptor_t):cudnnStatus_t; winapi; external libname;
  function cudnnBackendDestroyDescriptor(descriptor: cudnnBackendDescriptor_t):cudnnStatus_t; winapi; external libname;
  function cudnnBackendInitialize(descriptor: cudnnBackendDescriptor_t):cudnnStatus_t; winapi; external libname;
  function cudnnBackendFinalize(descriptor: cudnnBackendDescriptor_t):cudnnStatus_t; winapi; external libname;
  function cudnnBackendSetAttribute(descriptor: cudnnBackendDescriptor_t; attributeName: cudnnBackendAttributeName_t; attributeType: cudnnBackendAttributeType_t; elementCount: int64; const arrayOfElements: Pointer):cudnnStatus_t; winapi; external libname;
  function cudnnBackendGetAttribute(const descriptor: cudnnBackendDescriptor_t; attributeName: cudnnBackendAttributeName_t; attributeType: cudnnBackendAttributeType_t; requestedElementCount: int64; elementCount: Pint64; arrayOfElements: Pointer):cudnnStatus_t; winapi; external libname;
  function cudnnBackendExecute(handle: cudnnHandle_t; executionPlan: cudnnBackendDescriptor_t; variantPack: cudnnBackendDescriptor_t):cudnnStatus_t; winapi; external libname;
  function cudnnBackendPopulateCudaGraph(handle: cudnnHandle_t; executionPlan: cudnnBackendDescriptor_t; variantPack: cudnnBackendDescriptor_t; graph: cudaGraph_t):cudnnStatus_t; winapi; external libname;
  function cudnnBackendUpdateCudaGraph(handle: cudnnHandle_t; executionPlan: cudnnBackendDescriptor_t; variantPack: cudnnBackendDescriptor_t; graph: cudaGraph_t):cudnnStatus_t; winapi; external libname;
