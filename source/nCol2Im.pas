...
procedure im2col_new(data_im: PSingle; const channels, height, width,
  kernel_h, kernel_w, pad_h, pad_w, stride_h, stride_w, dilation_h,
  dilation_w: longInt; data_col: PSingle);
...
implementation
uses {$ifdef USE_MULTITHREADING}steroids,{$endif} ntensors
  {$ifdef USE_TELEMETRY}, nBaseLayer{$endif};

{$if not declared(PMPParams)}
type
  PMPParams = ^ TMPParams;
  TMPParams = record
...
type
  ...
  procedure i2c(const f,t:IntPtr;const ptr:pointer=nil);
  var
      c, y, x: longint;
...
//    writeln('c2i');
    ...
    filldWord(data_im[0], height * width * channels, 0);
...
procedure im2col_new(data_im: PSingle; const channels, height, width,
  kernel_h, kernel_w, pad_h, pad_w, stride_h, stride_w, dilation_h,
  dilation_w: longInt; data_col: PSingle);
var x, y, zx, zy, k_w, k_h, i, c:SizeInt;
begin
  ...
  for c:= 0 to channels-1 do begin
    for y:= pad_h to height div stride_h - 2*pad_h-1 do
      for x:= pad_w to width div stride_w - 2*pad_w-1 do
        for zy := -k_h to k_h do
          for zx := -k_w to k_w do begin
            data_col[i] := data_im[c*width*height + (y*stride_h + zy*dilation_h)*width + x*stride_w + zx*dilation_w];
            inc(i)
          end;
  end;
